...
type EventQuery struct {
	...
	// eager-loading edges.
	withUser         *UserQuery
	withClientserver *ClientServerQuery
	withFKs          bool
...
func (eq *EventQuery) QueryClientserver() *ClientServerQuery {
	query := &ClientServerQuery{config: eq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := eq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (eq *EventQuery) Clone() *EventQuery {
	...
	return &EventQuery{
		...
		withClientserver: eq.withClientserver.Clone(),
...
// WithClientserver tells the query-builder to eager-load the nodes that are connected to
// the "clientserver" edge. The optional arguments are used to configure the query builder of the edge.
func (eq *EventQuery) WithClientserver(opts ...func(*ClientServerQuery)) *EventQuery {
	query := &ClientServerQuery{config: eq.config}
	for _, opt := range opts {
		opt(query)
	}
	eq.withClientserver = query
	return eq
}
...
func (eq *EventQuery) sqlAll(ctx context.Context) ([]*Event, error) {
	var (
		...
		loadedTypes = [2]bool{
			eq.withUser != nil,
			eq.withClientserver != nil,
		}
	)
	if eq.withUser != nil || eq.withClientserver != nil {
		withFKs = true
	}
	if withFKs {
	...
	if query := eq.withClientserver; query != nil {
		ids := make([]int, 0, len(nodes))
		nodeids := make(map[int][]*Event)
		for i := range nodes {
			if fk := nodes[i].event_clientserver; fk != nil {
				ids = append(ids, *fk)
				nodeids[*fk] = append(nodeids[*fk], nodes[i])
			}
		}
		query.Where(clientserver.IDIn(ids...))
		neighbors, err := query.All(ctx)
		if err != nil {
			return nil, err
		}
		for _, n := range neighbors {
			nodes, ok := nodeids[n.ID]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "event_clientserver" returned %v`, n.ID)
			}
			for i := range nodes {
				nodes[i].Edges.Clientserver = n
			}
		}
	}
