...
func (uq *UserQuery) QueryOwns() *ClientServerQuery {
	query := &ClientServerQuery{config: uq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := uq.prepareQuery(ctx); err != nil {
			return nil, err
		}
	...
	if query := uq.withOwns; query != nil {
		...
		_spec := &sqlgraph.EdgeQuerySpec{
			...
			Predicate: func(s *sql.Selector) {
				s.Where(sql.InValues(user.OwnsPrimaryKey[1], fks...))
			},
		...
		if err := sqlgraph.QueryEdges(ctx, uq.driver, _spec); err != nil {
			return nil, fmt.Errorf(`query edges "owns": %v`, err)
		}
		query.Where(clientserver.IDIn(edgeids...))
		neighbors, err := query.All(ctx)
		if err != nil {
			return nil, err
		}
		for _, n := range neighbors {
			nodes, ok := edges[n.ID]
			if !ok {
				return nil, fmt.Errorf(`unexpected "owns" node returned %v`, n.ID)
			}
			for i := range nodes {
				nodes[i].Edges.Owns = append(nodes[i].Edges.Owns, n)
			}
		}
	}
