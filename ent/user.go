...
func (*User) scanValues(columns []string) ([]interface{}, error) {
	...
	for i := range columns {
		switch columns[i] {
		case user.FieldID, user.FieldUserNumber:
			values[i] = &sql.NullInt64{}
		case user.FieldUserID, user.FieldUserPw, user.FieldUserName, user.FieldEmail:
...
func (u *User) assignValues(columns []string, values []interface{}) error {
	...
	for i := range columns {
		...
		case user.FieldUserPw:
			if value, ok := values[i].(*sql.NullString); !ok {
				return fmt.Errorf("unexpected type %T for field user_pw", values[i])
			} else if value.Valid {
		...
		case user.FieldUserNumber:
			if value, ok := values[i].(*sql.NullInt64); !ok {
				return fmt.Errorf("unexpected type %T for field user_number", values[i])
			} else if value.Valid {
				u.UserNumber = int(value.Int64)
			}
...
func (u *User) String() string {
	...
	builder.WriteString(fmt.Sprintf("%v", u.UserNumber))
